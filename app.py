...
with col_wh:
  selected_warehouse = st.selectbox(
      "Warehouse", options=["AUH1", "DXB5", "DXB3"], index=0
  )
...
def analyze_mispunches(row, punch_cols):
  ...
  return pd.Series(
      [total_punches, working_hours_str, status, category, issue_type]
  )
...
if uploaded_file is not None:
  ...
  base_info_df["Repeated\nOffender"] = (
      base_info_df.groupby("P.Soft ID").cumcount() + 1
  )
  ...
  defaulter_hours_only = final_df[
      final_df["Issue Type"] == "Defaulter Hours"
  ].copy()
  ...
  if st.session_state.selected_view == "defaulters":
    st.subheader(
        f"⏰ Defaulter Working Hours List ({len(defaulter_hours_only)} Records)"
    )
    st.caption("Net working hours < 08:50 or > 09:10 wale employees ki list:")
    if len(defaulter_hours_only) > 0:
      st.dataframe(
          defaulter_hours_only.drop(columns=["Issue Type"]),
    ...
    st.subheader(
        f"⚠️ Missing & Extra Punches List ({len(mispunches_only)} Records)"
    )
    st.caption(
        "Missing punches (1, 3, 5), last scan IN, ya Extra Scans (7+) wale"
        " employees ki list:"
    )
    if len(mispunches_only) > 0:
      mispunch_display_df = mispunches_only.drop(
          columns=["Issue Type", "No. of\nWorking Hours"]
      )
      st.dataframe(
          mispunch_display_df,
          column_config=column_config_settings,
    ...
    st.subheader(
        f"✅ Clean Employee Records ({len(clean_records_only)} Records)"
    )
    st.caption("Pura time aur exact punches wale perfect records:")
    st.dataframe(
        clean_records_only.drop(columns=["Issue Type"]),
        column_config=column_config_settings,
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.dataframe(
        final_df.drop(columns=["Issue Type"]),
        column_config=column_config_settings,
        use_container_width=True,
        hide_index=True,
    )
  ...
  def convert_df(df_to_export):
    ...
    try:
      with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df_to_export.drop(columns=["Issue Type"]).to_excel(
            writer, index=False, sheet_name="Summary Report"
        )
        mispunches_only.drop(columns=["Issue Type"]).to_excel(
            writer, index=False, sheet_name="Mispunches"
        )
        defaulter_hours_only.drop(columns=["Issue Type"]).to_excel(
            writer, index=False, sheet_name="Defaulter Hours"
        )
    except Exception:
