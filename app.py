...
def analyze_mispunches(row, punch_cols, mode):
    ...
    if total_punches == 1:
        ...
        if "Day Shift Only" in mode:
            if punch_total_mins >= 17 * 60:
                return pd.Series([1, shift_val, "N/A", "OK", "Night Shift Start Check-in (Ignored)", "Clean"])
            else:
                return pd.Series([1, shift_val, "N/A", "Error", "Single Scan Only (Missing Shift OUT/Breaks)", "Mispunch"])
                
        elif "Night Shift Only" in mode:
            if 6 * 60 <= punch_total_mins <= 12 * 60:
                return pd.Series([1, shift_val, "N/A", "OK", "Day Shift Start Check-in (Ignored)", "Clean"])
            else:
                return pd.Series([1, shift_val, "N/A", "Error", "Single Scan Only (Missing Shift OUT/Breaks)", "Mispunch"])
    ...
    if total_punches % 2 != 0 or is_last_punch_an_in:
        ...
        return pd.Series([total_punches, shift_val, "N/A", "Error", category, "Mispunch"])

    dummy_date = datetime(2026, 1, 1)
    total_duration_seconds = sum(
        (datetime.combine(dummy_date, punches[i+1]) - datetime.combine(dummy_date, punches[i])).total_seconds() 
    ...
    # 9 Hours Rule (9 * 3600 = 32400 seconds)
    if effective_seconds >= 32400:
        return pd.Series([total_punches, shift_val, hours_str, "OK", "Complete", "Clean"])
    else:
        return pd.Series([total_punches, shift_val, hours_str, "Error", "Short Working Hours (< 9h target)", "Defaulter Hours"])
